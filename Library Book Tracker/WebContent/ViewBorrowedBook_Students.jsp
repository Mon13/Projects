...
<% } else { %>
   ...
   <% } %>
    ...
    <div id="body">
        ...
        <article>
        <h3>List of Students and books borrowed by them</h3>
        
        <%
			UserDAO dataobject1= new UserDAO();
       		List<BookList> result = new ArrayList();
       		result=dataobject1.getStudentsFined();
       		String htmlpage="\n";
			...
			for(int i=0;i<result.size();i++)
            {
                    ...
                    htmlpage+= "<h4>";
                 	htmlpage+=result.get(i).getUser().getFirstName()+"  "+result.get(i).getBook().getBookTitle()+"  "+result.get(i).getCheckedbook().getDueDate()+"  "+result.get(i).getCheckedbook().getCheckoutDate(); //CHECK THIS THREE DIFFERENT BEAN 
                    htmlpage+= "</h4><br>";
